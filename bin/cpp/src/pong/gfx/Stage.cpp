...
Stage_obj::Stage_obj()
{
	InitMember(_displayObjects);
}

void Stage_obj::__Mark()
{
	MarkMember(_displayObjects);
}
...
Dynamic Stage_obj::__SetField(const String &inName,const Dynamic &inValue)
{
	...
	case 9:
		...
		break;
	case 15:
		if (!memcmp(inName.__s,L"_displayObjects",sizeof(wchar_t)*15) ) { _displayObjects=inValue.Cast<Array<Dynamic > >();return inValue; }
	}
...
{
	outFields->push(STRING(L"_displayObjects",15));
